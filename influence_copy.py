...
def get_loss_gradient(model, loss, test_data):
    user, item = test_data
...
def predict_gradient(test_data, model):
    ...
    for g in grads:
        if i == 0:
            grad.append(g[user][0])
        elif i == 1:
            grad.append(g[item][0])
        elif i == 2:
            grad.append(g[user][0])
        elif i == 3:
            grad.append(g[item][0])
...
def get_influence(loss_func, train_data, test_data, model):
    ...
    grad_loss = get_loss_gradient(model, loss, test_data)
